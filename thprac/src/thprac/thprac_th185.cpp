...
#define BOSS_SPELL_CARD_ASYNC(boss_id, spell_id) 0x00, 0x00, 0x00, 0x00, 0x0B, 0x00, 0x24, 0x00, 0x00, 0x00, 0xFF, 0x01, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x42, 0x6F, 0x73, 0x73, boss_id / 10 + 0x30, boss_id % 10 + 0x30, 0x42, 0x6F, 0x73, 0x73, 0x43, 0x61, 0x72, 0x64, 0x30 + spell_id, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x10, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00
        ...
        auto ForceWave = [](const char* sub_name, std::vector<const char*> wave_names, size_t off) -> stage_warps_t {
            stage_warps_t ret = {
                .label = "Wave",
                .type = stage_warps_t::TYPE_COMBO
            };
            
            ret.section_param.push_back({ .label = "None (Random)" });

            for (size_t i = 0; i < wave_names.size(); i++) {
                section_param_t section_param = {
                    .label = wave_names[i]
                };
                std::vector<ecl_write_t> writes;
                for (size_t j = 0; j < wave_names.size(); j++) {
                    if (i != j) {
                        writes.push_back({ .off = off + 4 + 0x18 * j, .bytes = { 0, 0 } });
                    }
                }
                section_param.writes[sub_name] = writes;
                ret.section_param.push_back(section_param);
            }
        ...
        std::vector<const char*> s1Waves = {
            "FairyWave Type 2way",
            "FairyWave Type Aim 1",
            "FairyWave Rainy Bule",
            "FairyWave Rainy Green",
            "FairyWave Side Attack Red",
            "FairyWave Side Attack Shine",
            "FairyWave Back Attack Spread",
            "FairyWave Back Attack AIM",
            "Big Fairy Splash",
            "Big Fairy Machine Gun",
            "Big Fairy Red Flower",
            "Great Fairy Flower Cyclone"
        };
        std::vector<const char*> s2Waves = {
            "Wheel Ghost Circle",
            "Wheel Ghost Wave",
            "Wheel Ghost Circle 2",
            "Wheel Ghost Wave 2",
            "Chief Fairy Marching",
            "Chief Fairy Duel",
            "Chief Fairy Kunai",
            "Wheel Ghost Rolling",
            "Big Fairy Kikyo",
            "Great Fairy Narrow Way"
        };
        std::vector<const char*> s3Waves = {
            "Onibi Sadly Drop",
            "Onibi Across River",
            "Onibi Spread All Over",
            "Onibi Congestion",
            "Haunted Place Red",
            "Haunted Place Blue",
            "Onibi Back Attack",
            "Lingering Onibi",
            "Great Fairy Narrow Way 2",
            "Big Fairy Flash Gun",
            "Big Fairy Whipping Green"
        };
        ...
        case 1:
            ...
            out.section_param[0].phases = ForceWave("WorldWave01", s1Waves, 0x34);
            out.section_param[1].phases = ForceWave("WorldWave01", s1Waves, 0x34);
            out.section_param[2].phases = ForceWave("WorldWave01b", s1Waves, 0x34);
        ...
        case 2:
            ...
            out.section_param[0].phases = ForceWave("WorldWave01", s2Waves, 0x34);
            out.section_param[1].phases = ForceWave("WorldWave01", s2Waves, 0x34);
            out.section_param[2].phases = ForceWave("WorldWave01b", s2Waves, 0x34);
            out.section_param[3].phases = ForceWave("WorldWave01", s2Waves, 0x34);
            out.section_param[4].phases = {
                .label = "Boss",
                .type = stage_warps_t::TYPE_COMBO,
                .section_param = {
        ...
        case 3:
            ...
            out.section_param[0].phases = ForceWave("WorldWave01", s3Waves, 0x34);
            out.section_param[1].phases = ForceWave("WorldWave01", s3Waves, 0x34);
            out.section_param[2].phases = ForceWave("WorldWave01b", s3Waves, 0x34);
            out.section_param[3].phases = ForceWave("WorldWave01", s3Waves, 0x34);
